...
# ============================================================
# NACA 4412 structured supervisor-topology mesh
#
...
out_msh = Path("naca4412_v9_grid_COARSE.msh")
out_png = Path("naca4412_v9_grid_COARSE_preview.png")
...
# Mesh resolution
# -----------------------------
n_nose_half = 70         # split -> LE and LE -> split
n_airfoil_upper = 150     # split -> TE
n_transition = 15         # TE -> wake throat
n_wake = 160              # shared wake centreline -> outlet, V12
...
def naca4_thickness(xc, thickness):
    """
    Closed-trailing-edge NACA four-digit thickness distribution.
    The -0.1036 coefficient gives zero thickness at x/c = 1.
    """
...
def naca4_camber(xc, maximum_camber, camber_position):
    """
    NACA four-digit mean camber line and chordwise derivative.
    """
    xc = np.asarray(xc, dtype=float)
...
def naca4412_surface_points(xc):
    ...
    Thickness is applied normal to the mean camber line.
    """
    xc = np.atleast_1d(np.asarray(xc, dtype=float))

    if np.any(xc < 0.0) or np.any(xc > 1.0):
        raise ValueError(
            "NACA chordwise parameter must lie in [0, 1]"
        )
    ...
    xu = xc - yt * np.sin(theta_camber)
    yu = yc + yt * np.cos(theta_camber)

    xl = xc + yt * np.sin(theta_camber)
    yl = yc - yt * np.cos(theta_camber)
...
# -----------------------------
# NACA4412 airfoil construction
# -----------------------------

# x_split is a nondimensional NACA chordwise parameter.
# Because thickness is applied normal to the camber line,
# the physical x coordinates of the upper and lower split
# points are slightly different.

split_upper, split_lower = naca4412_surface_points(
    np.array([x_split])
)

p_split_u = split_upper[0].copy()
p_split_l = split_lower[0].copy()
...
p_top_split = np.array([p_split_u[0], y_top])
p_bottom_split = np.array([p_split_l[0], y_bottom])
...
# Nose inner: lower split -> LE -> upper split.
# Retain the V9 leading-edge clustering.

xc_lower_nose = one_sided_cosine_segment(
    x_split,
    x_le,
    n_nose_half,
    cluster="end",
...
_, lower_nose = naca4412_surface_points(xc_lower_nose)

xc_upper_nose = one_sided_cosine_segment(
    x_le,
    x_split,
    n_nose_half,
    cluster="start",
...
upper_nose, _ = naca4412_surface_points(xc_upper_nose)

# Exact structured interfaces.
lower_nose[0] = p_split_l
lower_nose[-1] = p_le

upper_nose[0] = p_le
upper_nose[-1] = p_split_u

nose_inner = np.vstack([
    lower_nose,
    upper_nose[1:],
])


# Inner C-shaped farfield.
#
# The cambered airfoil gives slightly different physical x coordinates
# at the upper and lower split points. Apply a smooth x-shift around
# the C-arc so both structured interfaces meet exactly.
...
outer_fraction = np.linspace(
    0.0,
    1.0,
    nose_inner.shape[0],
)

lower_outer_shift = p_split_l[0] - x_split
upper_outer_shift = p_split_u[0] - x_split

outer_x_shift = (
    (1.0 - outer_fraction) * lower_outer_shift
    + outer_fraction * upper_outer_shift
)

nose_outer = np.column_stack([
    x_c + r_far * np.cos(theta) + outer_x_shift,
    r_far * np.sin(theta),
])


# Upper surface: split -> sharp TE.
xc_upper_main = np.linspace(
    x_split,
    x_te,
    n_airfoil_upper,
)

upper_airfoil, _ = naca4412_surface_points(xc_upper_main)


# Lower surface: split -> sharp TE.
xc_lower_main = np.linspace(
    x_split,
    x_te,
    n_airfoil_upper,
)

_, lower_airfoil = naca4412_surface_points(xc_lower_main)


# Enforce exact block-interface and sharp-TE coordinates.
upper_airfoil[0] = p_split_u
lower_airfoil[0] = p_split_l

upper_airfoil[-1] = p_te
lower_airfoil[-1] = p_te


# Geometry/interface sanity checks.
if not np.allclose(
    nose_inner[0],
    p_split_l,
    atol=1.0e-14,
    ...
    raise RuntimeError(
        "Lower nose/split interface does not match"
    )
...
if not np.allclose(
    ...
    raise RuntimeError(
        "Upper nose/split interface does not match"
    )

if not np.allclose(
    nose_outer[0],
    np.array([p_split_l[0], y_bottom]),
    atol=1.0e-12,
):
    raise RuntimeError(
        "Lower outer split interface does not match"
    )

if not np.allclose(
    nose_outer[-1],
    np.array([p_split_u[0], y_top]),
    atol=1.0e-12,
):
    raise RuntimeError(
        "Upper outer split interface does not match"
    )
...
if not np.allclose(
    ...
    raise RuntimeError(
        "Upper and lower surfaces do not close at the sharp TE"
    )
...
dx_te = xc_upper_main[-1] - xc_upper_main[-2]
...
# Outer C-shaped inlet: retain the V9 angular distribution,
# but preserve the small NACA4412 split-point x offsets.

nose_far_outer = np.column_stack([
    x_c + r_far_outer * np.cos(theta) + outer_x_shift,
    r_far_outer * np.sin(theta),
])


# Map upper and lower streamwise coordinates independently.
#
# For NACA0012 these mappings were identical because the airfoil
# was symmetric. For NACA4412 the physical upper/lower x locations
# differ slightly upstream of the sharp TE.

upper_stream_s = (
    (upper_outer[:, 0] - p_split_u[0])
    / (x_out - p_split_u[0])
)

lower_stream_s = (
    (lower_outer[:, 0] - p_split_l[0])
    / (x_out - p_split_l[0])
)


# Retain the validated V9 interface-transition function.
...
top_far_outer_x = (
    p_split_u[0]
    + upper_stream_f
    * (x_out_outer - p_split_u[0])
)

bottom_far_outer_x = (
    p_split_l[0]
    + lower_stream_f
    * (x_out_outer - p_split_l[0])
)


top_far_outer = np.column_stack([
    top_far_outer_x,
    np.full(top_far_outer_x.shape[0], y_top_outer),
])

bottom_far_outer = np.column_stack([
    bottom_far_outer_x,
    np.full(bottom_far_outer_x.shape[0], y_bottom_outer),
])
...
plt.title("Structured NACA 4412 V9 sharp-TE C-grid mesh preview")
plt.xlabel("x/c")
plt.ylabel("y/c")
plt.tight_layout()
